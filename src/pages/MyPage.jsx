...
const MyPage = () => {
  ...
  useEffect(() => {
      setFilteredData(data.filter(marker => 
  ...
  useEffect(() => {
    ...
    async function fetchPlaces() {
      ...
      for (let i = 0; i < favoriteList.length; i++) {
        const data = await axios.get(`https://corsproxy.io/?${encodeURIComponent(`https://place.map.kakao.com/main/v/${favoriteList[i]}`)}`)
        _data.push({
          name: data.data.basicInfo.placenamefull,
          category_name: `${data.data.basicInfo.category.cate1name ?? ''} > ${data.data.basicInfo.category.catename ?? ''}`,
              ...
              <div className="grid grid-cols-5 max-2xl:grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 gap-10 box-border h-[74vh] overflow-y-auto p-5">
                {
                  filteredData.map((data, index) => (
                    <div key={index} className="card max-w-xs h-96 bg-violet-50 shadow-xl mb-8">
                      <Link to={data.url} className="flex flex-col items-center">
                        <img src={data.imgUrl} alt={data.name} className="w-full h-48 object-cover cursor-pointer" onClick={() => window.open(data.url, "_blank")} />
                        <div className="card-title justify-center mt-4 px-4">{data.name}</div>
                      </Link>
